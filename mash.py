...
def main():
    ...
    parser = OptionParser(usage, version='%prog 0.6.17')
    parser.add_option("-o", "--outputdir", default="", dest="outputdir",
                      help="output directory")
    parser.add_option(
        "-c", "--config", default="/etc/mash/mash.conf", dest="config",
    ...
    if opts.outputdir != "":
        if opts.outputdir[0] != '/':
            opts.outputdir = os.path.realpath(
                os.path.join(os.getcwd(), opts.outputdir))
    ...
    if opts.previous != "":
        if opts.previous[0] != '/':
            opts.previous = os.path.realpath(
                os.path.join(os.getcwd(), opts.previous))
        for dist in conf.distros:
